 ...
 */
public class ConsonantAnimationFragment extends Fragtivity {

    public Factory.Alphabets.Consonant consonant;
    private int textColor, borderColor;
    ArialTextView centerText, topTextStub;
    LinearLayout centerTextLayout, text;
    ImageView image;

    public static ConsonantAnimationFragment getInstance(int textColor, int borderColor, Factory.Alphabets.Consonant consonant){
        return new ConsonantAnimationFragment().setInstanceStuff(textColor, borderColor, consonant);
    }

    private ConsonantAnimationFragment setInstanceStuff(int textColor, int borderColor, Factory.Alphabets.Consonant consonant){
    ...
    void animateToTop(){
        Log.d("Animation to top");
        int height = topTextStub.getHeight();
        int currentHeight = centerTextLayout.getHeight();
        float scale = (float)height / (float)currentHeight;
        int[] locs = new int[2];
        topTextStub.getLocationInWindow(locs);
        float y = locs[1] - height/2;
        Log.d("Moving to location : y : " + y);

        PropertyValuesHolder ty = PropertyValuesHolder.ofFloat("Y", centerTextLayout.getY(), y);
        ...
        ValueAnimator animator = ObjectAnimator.ofPropertyValuesHolder(centerTextLayout, ty, sx, sy);
    ...
    void animateImage(){
        ...
        char[] chars = consonant.accompanyingText.toCharArray();
        for(char ch : chars){
            final ArialTextView textView = new ArialTextView(getContext());
            String val = String.valueOf(ch);
            textView.setTextSize(TypedValue.COMPLEX_UNIT_PX, getResources().getDimensionPixelSize(R.dimen.text_size_display3));
            textView.setTextColor(consonant.value.equalsIgnoreCase(val) ? Color.BLACK : Color.WHITE );
            LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
            params.gravity = Gravity.CENTER;
            textView.setLayoutParams(params);
        ...
        for(int i = 0; i < text.getChildCount(); i++){
            Log.d("Animation child at " + i);
            View child = text.getChildAt(i);
            child.setAlpha(0f);
            child.setVisibility(View.VISIBLE);
            ValueAnimator textAnimator = ViewAnimator.fadeIn(child, i * 50, 200);
            if(i == text.getChildCount()-1) {
    ...
    void playAccompanyingText(){
        for(int i = 0; i < text.getChildCount(); i ++){
            View child = text.getChildAt(i);
            ValueAnimator animator = ObjectAnimator.ofFloat(child, "translationY", child.getTranslationY(), (child.getTranslationY() -10f));
            animator.setInterpolator(new AccelerateDecelerateInterpolator());
